...
def draw(dataSet, tree, title, loc='left', color='red', drawEnd=True):
  ...
  plt.xlabel('Time')
  plt.ylabel('收盘价')
  plt.scatter(dataSet[:, 0], dataSet[:, 1], s=5, marker=".")  # 在图中以点画收盘价
...
if __name__ == '__main__':
  ...
  for c in mp.colors.cnames:
    ...
    difftime = round(end - start, 4)
    draw(arr, tree,
         stCode + ' start: ' + stDate + " TotalTime:" + str(difftime) + " " + str(
           round(difftime / len(df), 4)) + " " + str(len(e)), color=c)
    # i+=1
    if end - start2 < 5:
